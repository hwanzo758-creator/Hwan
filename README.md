...
- Bicubic, SRCNN, EDSR, RCAN, SwinIR-based SR, ESRGAN-based SR, HAT-based SR, DAT-based SR, and DRCT-based SR
...
`Final_version6_drct.py` expects a preprocessed dataset directory like this:
...
```text
.
|-- Final_version6_drct.py      main training / evaluation pipeline
|-- selftest_drct.py           pre-flight check for the DRCT baseline
...
| File | Purpose |
|---|---|
| `Final_version6_drct.py` | Main training/evaluation pipeline for all nine methods and every revision experiment. Contains the data-integrity filters, patch-quality thresholds, valid-pixel masking, all metric implementations, and the fixed random seed. |
| `selftest_drct.py` | Pre-flight check for the DRCT baseline: parameter counts, forward/backward finiteness, peak memory and step time for the DRCT baseline |
...
```bash
SR_PRESET=revision_experiments SR_SCALES=2,3,4 python -u Final_version6_drct.py
```
...
| DRCT baseline only, all scales and both modes | `SR_PRESET=drct_only SR_SCALES=2,3,4` |
| DRCT loss-term ablation at x4 | `SR_PRESET=loss_ablation_drct SR_SCALES=4` |
| DRCT noise robustness at x4 | `SR_PRESET=noise_drct SR_SCALES=4 SR_NOISE_METHODS=drct` |

Implemented revision components:

- `hat`, `dat` and `drct`: recent SR baselines added during revision (HAT CVPR 2023, DAT ICCV 2023, DRCT CVPR Workshops 2024)
...
| `drct` | DRCT-based dense-residual-connected transformer SR baseline added in the revision |
...
# a single method (example: the DRCT baseline)
SR_PRESET=drct_only SR_SCALES=2,3,4 python -u Final_version6_drct.py

# loss-term ablation (Table 7)
SR_PRESET=loss_ablation_hatdat  SR_SCALES=4 python -u Final_version6_drct.py
SR_PRESET=loss_ablation_drct SR_SCALES=4 python -u Final_version6_drct.py

# alternative degradation (Table 8)
SR_DEGRADATION=blur_bicubic SR_PRESET=revision_experiments SR_SCALES=4 python -u Final_version6_drct.py

# noise robustness (Table 9), evaluation only - reuses the x4 checkpoints
SR_PRESET=noise_hat      SR_SCALES=4 python -u Final_version6_drct.py
SR_PRESET=noise_drct  SR_SCALES=4 SR_NOISE_METHODS=drct python -u Final_version6_drct.py
```

Recognised environment variables: `SR_DATASET_DIR`, `SR_OUTPUT_ROOT`, `SR_RUN_NAME`, `SR_PRESET`,
`SR_SCALES`, `SR_DEGRADATION`, `SR_NOISE_METHODS`, and the DRCT shape knobs
`SR_DRCT_DIM / _GROWTH / _DEPTH / _GROUPS / _HEADS`.

## Where the Reviewer-Requested Items Live in the Code

| Item | Location in `Final_version6_drct.py` |
|---|---|
