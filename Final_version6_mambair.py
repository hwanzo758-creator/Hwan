...
# REVISION (B5 deploy): every path/preset below can be overridden by an
# environment variable, so the same file can be shipped to the server and driven
# by run_mambair.sh without editing the source. Defaults are unchanged.
...
RUN_NAME = os.environ.get("SR_RUN_NAME", "final_srcnn_improved_train_missing_ckpt")
...
# If the configured checkpoint path is missing, search existing runs under OUTPUT_ROOT.
# This is only used for loading. New checkpoints are still saved to get_checkpoint_path().
AUTO_FIND_CHECKPOINTS = True
CHECKPOINT_SEARCH_ROOT = OUTPUT_ROOT
CHECKPOINT_SEARCH_PREFER_RUNS = [
    RUN_NAME,
    CHECKPOINT_RUN_NAME,
    ...
    "final_train_only_retrain_all_no_ckpt_skip",
    "final_srcnn_improved_train_missing_ckpt",
]
...
# - EVAL_SAMPLE_SIZE = 100  : evaluate 100 randomly selected scenes
# - EVAL_SAMPLE_SIZE = None : evaluate the full selected source
...
# CPU parallelism for metric calculation during evaluation.
# Metric workers speed up SSIM/SAM/VI/band metrics, while GPU inference remains serial.
EVAL_METRIC_PARALLEL = True
EVAL_METRIC_CPU_WORKERS = min(4, max(1, os.cpu_count() or 1))
TORCH_CPU_THREADS = max(1, min(8, os.cpu_count() or 1))
EVAL_TILE_BATCH_SIZE = 16  # RTX PRO 6000 Blackwell 97GB VRAM
...
#
EXPERIMENT_PRESET = os.environ.get("SR_PRESET", "full_eval")
...
USER_SCALES = [int(v) for v in os.environ.get("SR_SCALES", "4").split(",") if v.strip()]
...
PRESETS = {
    ...
    # ========================================================
    # REVISION: all reviewer-requested experiments in one preset.
    # Set USER_SCALES = [2, 4]. New models (hat, edsr_wide) train; existing
    # methods are skipped if their checkpoints/metrics already exist. Per-method
    # scale/mode limits (REVISION_METHOD_SCALES/MODES) keep edsr_wide to x4-joint
    # and hat to x2/x4 both modes. A1 runs via the loss-ablation override below
    # (edsr joint, "spatial_no_spectral" profile). B3 noise runs after the main
    # loop. B2 (blur+bicubic) is run as a SEPARATE invocation with
    # DEGRADATION_MODE="blur_bicubic" (see REVISION_RUN_GUIDE).
    # ========================================================
    "revision_experiments": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["bicubic", "srcnn", "edsr", "rcan", "swinir", "esrgan", "hat", "dat", "mambair", "edsr_wide"],
        "BANDWISE_METHODS_TO_RUN": ["bicubic", "edsr", "rcan", "swinir", "esrgan", "hat", "dat", "mambair"],
        "RUN_JOINT_5CH": True,
        "RUN_BANDWISE_ABLATION": True,
        "EPOCHS": 50,
        "PATCHES_PER_EPOCH": 1000,
    ...
    # ========================================================
    # REVISION B2 (R3 #4): degradation-robustness run. Use together with
    # DEGRADATION_MODE="blur_bicubic" and USER_SCALES=[4]. Runs one representative
    # per model family — CNN (EDSR, RCAN) and Transformer (HAT) — in joint +
    # band-wise, with the loss ablation (A1) and noise (B3) disabled (those belong
    # to the area run). Outputs auto-isolate to a "*_blur_bicubic" folder.
    # ========================================================
    "degradation_b2": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["bicubic", "edsr", "rcan", "hat"],
        "BANDWISE_METHODS_TO_RUN": ["bicubic", "edsr", "rcan", "hat"],
        "RUN_JOINT_5CH": True,
        "RUN_BANDWISE_ABLATION": True,
        "EPOCHS": 50,
        "PATCHES_PER_EPOCH": 1000,
    ...
    # REVISION: retrain a single diverged case (ESRGAN joint five-channel) at the
    # scale set by USER_SCALES. Delete the old joint5ch_esrgan_x{scale}_best.pth
    # and the metrics CSV first so it retrains and re-evaluates.
    "esrgan_retrain": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["esrgan"],
        "BANDWISE_METHODS_TO_RUN": [],
        "RUN_JOINT_5CH": True,
        "RUN_BANDWISE_ABLATION": False,
        "EPOCHS": 50,
        "PATCHES_PER_EPOCH": 1000,
    ...
    # REVISION: retrain a diverged band-wise case (e.g. HAT). Only the deleted
    # band checkpoints retrain; delete the metrics CSV so it re-evaluates.
    "hat_bandwise_retrain": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": [],
        "BANDWISE_METHODS_TO_RUN": ["hat"],
        "RUN_JOINT_5CH": False,
        "RUN_BANDWISE_ABLATION": True,
        "EPOCHS": 50,
        "PATCHES_PER_EPOCH": 1000,
    ...
    # REVISION (B5): train + evaluate only the MambaIR baseline, joint and
    # band-wise, at USER_SCALES = [2, 3, 4]. Existing methods are untouched;
    # their metrics/checkpoints are reused, so only MambaIR is trained.
    "mambair_only": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["mambair"],
        "BANDWISE_METHODS_TO_RUN": ["mambair"],
        "RUN_JOINT_5CH": True,
        "RUN_BANDWISE_ABLATION": True,
        "EPOCHS": 50,
        "PATCHES_PER_EPOCH": 1000,
    ...
    # REVISION (B5, Table 9): noise robustness for MambaIR at x4. No retraining -
    # the x4 checkpoints from "mambair_only" are reused and only the noisy-input
    # evaluation passes are run. Use together with SR_NOISE_METHODS=mambair.
    "noise_mambair": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["mambair"],
        "BANDWISE_METHODS_TO_RUN": [],
        ...
        "SAVE_DIFF_MAP": False,
        "SAVE_CHECKPOINT": True,
        "MAX_PREVIEW_PER_METHOD": 0,
    },

    # REVISION (B5, Table 7): loss-term ablation rows for MambaIR.
    "loss_ablation_mambair": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["mambair"],
        "BANDWISE_METHODS_TO_RUN": [],
        "RUN_JOINT_5CH": True,
    ...
    # REVISION (Table 7): loss-term ablation for the two added transformer
    # baselines (HAT, DAT), joint five-channel at USER_SCALES=[4]. Trains each
    # under L1, L1+SAM, and L1+SAM+VI so their rows can be appended to the
    # existing Section 3.6 loss-ablation table (Table 7) in the same format as
    # the other models. Band-wise and B3 noise are off; the full-loss baseline
    # is excluded (already reported in the main comparison).
    "loss_ablation_hatdat": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["hat", "dat"],
        "BANDWISE_METHODS_TO_RUN": [],
        "RUN_JOINT_5CH": True,
        "RUN_BANDWISE_ABLATION": False,
        "EPOCHS": 50,
    ...
    # REVISION (Table 9, B3): noise-robustness eval for the Transformer
    # representative (HAT) only. No training/main eval — just the additive-noise
    # evaluation on the existing HAT joint five-channel ×4 checkpoint, so the HAT
    # rows can be appended to the existing EDSR/RCAN/ESRGAN noise table.
    "noise_hat": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": [],
        "BANDWISE_METHODS_TO_RUN": [],
...
# ============================================================
# REVISION: SMOKE TEST switch
# ============================================================
# Set SMOKE_TEST = True for a fast end-to-end sanity check (few scenes, 1 epoch,
# few patches) to confirm the new models (hat, dat, edsr_wide), the A1 loss
# profile, and the B3 noise / D1-D3 reporting all run and produce CSVs.
# Set back to False for the real run. Overrides the preset's heavy settings.
SMOKE_TEST = False
if SMOKE_TEST:
    MAX_RECORDS_FOR_TEST = 30      # cap total scenes (train/val/test all shrink)
    EPOCHS = 1
    PATCHES_PER_EPOCH = 20
    print("[SMOKE_TEST] Fast sanity run: 30 scenes, 1 epoch, 20 patches/epoch.")
...
TRAIN_MODE = {
    ...
    "esrgan": "scratch",
    # REVISION additions
    "hat": "scratch",        # B4: recent SOTA baseline (HAT)
    "dat": "scratch",        # B4b: recent SOTA baseline (DAT)
    "mambair": "scratch",    # B5: recent SOTA baseline (MambaIR, ECCV 2024)
    "edsr_wide": "scratch",  # A3: parameter-matched widened joint EDSR
}
...
SKIP_EVAL_IF_METRICS_EXISTS = True
LOAD_EXISTING_METRICS_WHEN_SKIP_EVAL = True
...
# REVISION: fixed scale set for scene-quality filtering, independent of
# USER_SCALES, so every run (MAIN x2/x3/x4, B2 x4) produces the SAME valid-record
# set and therefore the SAME train/val/test split (n_test = 2170).
SCENE_QUALITY_SCALES = [2, 3, 4]
...
# REVISION (B4): HAT also uses global MultiheadAttention per tile, so it uses
# the same small inference tile as SwinIR to keep attention cost bounded.
HAT_TILE_LR_SIZE = 64
HAT_TILE_OVERLAP = 8

# REVISION (A3): widened joint EDSR configuration for the parameter-matched
# comparison. nf is chosen so the joint total parameter count approaches the
# band-wise total (~5x base joint EDSR nf=96). The build prints both counts so
# the value can be fine-tuned to match Table 2's band-wise total.
EDSR_WIDE_NF = 216
EDSR_WIDE_NBLOCKS = 16

# ============================================================
# REVISION EXPERIMENTS — master config
# ============================================================
# All reviewer-requested supplementary experiments are gated by these flags so
# they can be enabled together via the "revision_experiments" preset, or run
# selectively. Defaults below are inert (False / [0.0]) so importing this file
# does not change the behaviour of the original pipeline.

# D1 (R3#5): report standard PSNR (peak=1.0) — always computed in metrics dict.
# D3 (R3#9): also report VI errors with reflectance clipped to [0,1].
REPORT_VI_CLIP01 = True

# Per-method scale / mode restrictions for revision models (used by the run
# guard). Methods NOT listed here are allowed at every SCALE / mode as before.
#   edsr_wide (A3): joint five-channel, x4 only.
#   hat (B4):       joint + band-wise, x2 and x4.
REVISION_METHOD_SCALES = {
    "edsr_wide": [4],
    "hat": [2, 3, 4],
    "dat": [2, 3, 4],
    "mambair": [2, 3, 4],
}
REVISION_METHOD_MODES = {
    "edsr_wide": ["joint5ch"],
    "hat": ["joint5ch", "bandwise"],
    "dat": ["joint5ch", "bandwise"],
    "mambair": ["joint5ch", "bandwise"],
}

# B2 (R3#4): degradation used to generate LR inputs.
#   "area"        -> use the pre-generated area-downsampled LR files (default).
#   "blur_bicubic"-> generate LR on the fly as Gaussian blur + bicubic downsample.
# When set to "blur_bicubic", checkpoints/outputs/metrics are tagged with
# DEGRADATION_TAG so they never collide with the area-downsampling results.
# B2 degradation can be switched from the shell: SR_DEGRADATION=blur_bicubic
DEGRADATION_MODE = os.environ.get("SR_DEGRADATION", "area")
BLUR_BICUBIC_SIGMA = 1.5   # Gaussian blur sigma before bicubic downsampling
DEGRADATION_TAG = "" if DEGRADATION_MODE == "area" else f"_{DEGRADATION_MODE}"

# B3 (R3#6): additive Gaussian noise applied to x4 LR TEST inputs at eval time
# (no retraining). One evaluation pass is produced per sigma; sigma=0.0 is the
# clean reference. Rows are tagged with the noise level.
# Enabled automatically for the revision preset; inert otherwise.
if EXPERIMENT_PRESET == "revision_experiments":
    # Keep B3 quick during a smoke test, full set otherwise.
    EVAL_NOISE_SIGMAS = [0.0, 0.03] if globals().get("SMOKE_TEST", False) else [0.0, 0.01, 0.03]
elif EXPERIMENT_PRESET in ("noise_hat", "noise_mambair"):
    EVAL_NOISE_SIGMAS = [0.0, 0.01, 0.03]
else:
    EVAL_NOISE_SIGMAS = [0.0]
# B3 noise-robustness targets. Overridable from the shell so a newly added
# baseline can be appended to Table 9 without editing this file, e.g.
#   SR_NOISE_METHODS=mambair
EVAL_NOISE_METHODS = (
    [m.strip() for m in os.environ["SR_NOISE_METHODS"].split(",") if m.strip()]
    if os.environ.get("SR_NOISE_METHODS")
    else (["hat"] if EXPERIMENT_PRESET == "noise_hat" else ["edsr", "rcan", "esrgan"])
)
EVAL_NOISE_SCALES = [4]

# --- B2 isolation: when using an alternative degradation, redirect the run and
# checkpoint folders so blur+bicubic results never overwrite/reuse the area
# results, and disable checkpoint auto-search (which would otherwise find the
# area-trained checkpoints by filename). -------------------------------------
if DEGRADATION_MODE != "area":
    RUN_NAME = f"{RUN_NAME}{DEGRADATION_TAG}"
    CHECKPOINT_RUN_NAME = f"{CHECKPOINT_RUN_NAME}{DEGRADATION_TAG}"
    OUTPUT_DIR = OUTPUT_ROOT / RUN_NAME
    CHECKPOINT_DIR = OUTPUT_ROOT / CHECKPOINT_RUN_NAME / "sr_checkpoints"
    CHECKPOINT_SEARCH_ROOT = CHECKPOINT_DIR.parent
    AUTO_FIND_CHECKPOINTS = False
    print(f"[REVISION] DEGRADATION_MODE={DEGRADATION_MODE} -> isolated run dir: {RUN_NAME}")
...
# REVISION (A1, R3#1 / R4 L1.1): for the revision run, restrict the loss
# ablation to the single controlled experiment that isolates reconstruction mode
# from spectral loss: joint EDSR trained with spatial+detail losses only
# ("spatial_no_spectral"), compared against the full-loss joint EDSR baseline.
# (The original Section 3.6 ablation, Table 7, is unchanged and already exists.)
if EXPERIMENT_PRESET == "revision_experiments":
    LOSS_ABLATION_METHODS = ["edsr"]
    LOSS_ABLATION_MODES = ["joint5ch"]
    LOSS_ABLATION_INCLUDE_FULL_LOSS_BASELINE = True
    LOSS_ABLATION_PROFILE_IDS = ["spatial_no_spectral"]
elif EXPERIMENT_PRESET == "loss_ablation_hatdat":
    # Table 7 loss-term ablation for HAT and DAT (joint five-channel).
    LOSS_ABLATION_METHODS = ["hat", "dat"]
    LOSS_ABLATION_MODES = ["joint5ch"]
    LOSS_ABLATION_INCLUDE_FULL_LOSS_BASELINE = False
    LOSS_ABLATION_PROFILE_IDS = ["l1", "l1_sam", "l1_sam_vi"]
elif EXPERIMENT_PRESET == "loss_ablation_mambair":
    LOSS_ABLATION_METHODS = ["mambair"]
    LOSS_ABLATION_MODES = ["joint5ch"]
    LOSS_ABLATION_INCLUDE_FULL_LOSS_BASELINE = False
    LOSS_ABLATION_PROFILE_IDS = ["l1", "l1_sam", "l1_sam_vi"]
elif EXPERIMENT_PRESET in ("degradation_b2", "esrgan_retrain", "hat_bandwise_retrain", "noise_hat", "mambair_only", "noise_mambair"):
    # Pure retrain/degradation/noise-eval runs — no loss ablation.
...
LOSS_PROFILES = OrderedDict([
    ...
    # REVISION (A1, R3#1 / R4 L1.1): joint five-channel model trained with the
    # SAME spatial + detail losses used by the single-channel band-wise models
    # (i.e., SAM and VI removed). This isolates the effect of the reconstruction
    # MODE (joint vs band-wise) from the effect of the spectral loss terms.
    # Compare this against the "full" joint EDSR and the band-wise EDSR at x4.
    ("spatial_no_spectral", {
        "label": "Spatial+Detail (no SAM/VI)",
        "l1": L1_WEIGHT,
        "charbonnier": CHARBONNIER_WEIGHT,
        "ssim": SSIM_WEIGHT,
        "grad": GRAD_WEIGHT,
...
# REVISION: ESRGAN adversarial training diverged at x2 joint with adv=1e-3.
# Use a longer pixel-wise pretrain and a much smaller adversarial weight so the
# generator stays close to the content solution (stable; SAM ~1-2 deg). This
# matches the "compact adversarial baseline" framing.
ESRGAN_PRETRAIN_EPOCHS = 30
ESRGAN_ADV_WEIGHT = 1e-4
ESRGAN_L1_WEIGHT = 1.0
ESRGAN_SAM_WEIGHT = 0.05
ESRGAN_VI_WEIGHT = 0.05

# ESRGAN is more sensitive than CNN/Transformer SR models because the generator
# loss competes with the discriminator. Use smaller spectral/VI terms and a
# gentler adversarial phase for loss-ablation checkpoints.
ESRGAN_ABLATION_PRETRAIN_EPOCHS = 30
ESRGAN_ABLATION_ADV_WEIGHT = 2e-4
ESRGAN_ABLATION_SAM_WEIGHT = 0.02
ESRGAN_ABLATION_VI_WEIGHT = 0.02
ESRGAN_ABLATION_LOSS_RAMP_EPOCHS = 10
ESRGAN_ABLATION_CONTENT_CLAMP = 10.0

# Extra-safe settings used only for ESRGAN + L1 + SAM + VI. This profile was
# unstable with the normal VI loss, so the generator is trained as an RRDB
# content model and VI is introduced late with a stable denominator.
ESRGAN_L1_SAM_VI_PRETRAIN_EPOCHS = 999
ESRGAN_L1_SAM_VI_ADV_WEIGHT = 0.0
ESRGAN_L1_SAM_VI_WEIGHT = 0.003
ESRGAN_L1_SAM_VI_START_EPOCH = 20
ESRGAN_L1_SAM_VI_RAMP_EPOCHS = 30
ESRGAN_STABLE_VI_DENOM_EPS = 0.05
ESRGAN_STABLE_VI_BETA = 0.05
...
def revision_method_allowed(method, scale, mode):
    """REVISION: per-method scale/mode restrictions. Methods not listed in
    REVISION_METHOD_SCALES / REVISION_METHOD_MODES are unrestricted, so this has
    no effect on the original presets (edsr_wide and hat are the only entries)."""
    allowed_scales = REVISION_METHOD_SCALES.get(method)
    if allowed_scales is not None and scale not in allowed_scales:
        return False
    allowed_modes = REVISION_METHOD_MODES.get(method)
...
def write_dict_csv(path, rows):
    ...
    # REVISION: build fieldnames from the UNION of all row keys (order-preserving)
    # so heterogeneous rows are handled — e.g. new columns like PSNR_std and
    # *_MAE_clip01 that are absent from metric rows loaded from older CSVs.
    fieldnames = []
    seen = set()
    for row in rows:
        for k in row.keys():
    ...
    with open(path, "w", encoding="utf-8-sig", newline="") as f:
        # restval="" fills missing cells; extrasaction="ignore" is a safety net.
...
def read_scene_index():
    ...
    for r in records:
        ...
        if ENABLE_SCENE_QUALITY_FILTER:
            scene_is_bad = False
            scene_bad_rows = []
            # REVISION: use a FIXED quality-check scale set so the valid-record
            # set (and therefore the train/val/test split) is identical across
            # runs regardless of USER_SCALES. Without this, a run with
            # USER_SCALES=[4] (e.g. B2) filters more leniently than one with
            # [2,3,4] (MAIN), yielding a different test set (2293 vs 2170).
            quality_scales = SCENE_QUALITY_SCALES if CHECK_SCENE_QUALITY_ALL_SCALES else [SCALES[0]]
...
def generate_lr_from_hr(hr, scale, sigma=None):
    """REVISION (B2, R3#4): generate a low-resolution cube from an HR cube using
    the classical Gaussian-blur + bicubic-downsampling degradation, as an
    alternative to the area-based downsampling used elsewhere. Output dimensions
    match the area convention: floor(H/s) x floor(W/s). Blur is applied per band
    to avoid channel-count limitations in cv2."""
    if sigma is None:
        sigma = BLUR_BICUBIC_SIGMA
    ...
    for c in range(hr.shape[-1]):
        blurred = cv2.GaussianBlur(hr[..., c], ksize=(0, 0),
                                   sigmaX=float(sigma), sigmaY=float(sigma))
        lr_c = cv2.resize(blurred, (lr_w, lr_h), interpolation=cv2.INTER_CUBIC)
        bands.append(lr_c.astype(np.float32))

    return np.stack(bands, axis=-1)


def load_lr_cube(record, scale, hr_cube=None):
    """LR loader that honours DEGRADATION_MODE. For the default 'area' mode it
    loads the pre-generated LR file; for 'blur_bicubic' it derives the LR from
    the provided HR cube on the fly (used for the B2 degradation-robustness run)."""
    if DEGRADATION_MODE == "blur_bicubic":
        if hr_cube is None:
            hr_cube = load_npy(get_hr_path(record))
        return generate_lr_from_hr(hr_cube, scale)
    return load_npy(get_lr_path(record, scale))
...
def get_infer_tile_settings(method):
    ...
    # REVISION (B4/B4b): HAT and DAT use global attention per tile like SwinIR.
    if str(method).lower() in ("hat", "dat"):
        return int(HAT_TILE_LR_SIZE), int(HAT_TILE_OVERLAP)
    # REVISION (B5): MambaIR's state-space scan is linear in sequence length, so
    # it runs on the standard (large) inference tile like the CNN baselines.
...
class SRPatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # REVISION (B2): derive LR via blur+bicubic when that degradation is active.
        if DEGRADATION_MODE == "blur_bicubic":
            lr_raw = generate_lr_from_hr(np.asarray(hr_raw), self.scale)
        else:
...
# ============================================================
# REVISION (B4): HAT - Hybrid Attention Transformer (Chen et al., CVPR 2023)
# Compact reimplementation matched to the existing budget, following the same
# residual-SR template as the other models (head -> hybrid-attention groups ->
# body_conv -> global residual -> pixel-shuffle upsampler -> tail -> clamp).
# HAT's core idea is to combine channel attention with (self-)window attention
# in a single "hybrid attention block" (HAB); here the channel-attention branch
# reuses ChannelAttention and the token-mixing branch reuses global multi-head
# self-attention (as in SwinLikeBlock). Global attention is applied per tile,
# so HAT uses the same small inference tile as SwinIR (see HAT_TILE_LR_SIZE).
# ============================================================
class HABlock(nn.Module):
    """Hybrid Attention Block: self-attention branch + channel-attention branch."""
    ...
    def __init__(self, dim=96, heads=4, mlp_ratio=2.0, ca_weight=0.1):
        ...
        # Channel-attention branch (CAB), operating in image space.
...
class HybridAttentionGroup(nn.Module):
    """Residual hybrid-attention group (RHAG) of stacked HABlocks."""

    def __init__(self, dim=96, heads=4, depth=4):
        super().__init__()

        self.blocks = nn.Sequential(
            *[HABlock(dim=dim, heads=heads) for _ in range(depth)]
        )
...
class HAT(nn.Module):
    def __init__(self, scale, channels=5, dim=96, n_groups=4, depth=4, heads=4):
        ...
        self.body = nn.Sequential(
            *[HybridAttentionGroup(dim=dim, heads=heads, depth=depth)
...
# ============================================================
# REVISION (B4b): DAT - Dual Aggregation Transformer (Chen et al., ICCV 2023)
# Compact reimplementation matched to the existing budget, same residual-SR
# template. DAT's core idea is "dual aggregation": alternating SPATIAL
# self-attention (token mixing across pixels) and CHANNEL self-attention
# (transposed / MDTA-style attention across feature channels), which pairs well
# with this paper's spectral/channel emphasis. Global spatial attention is
# applied per tile, so DAT uses the same small inference tile as SwinIR/HAT.
# ============================================================
class ChannelSelfAttention(nn.Module):
    """Transposed (channel-wise) self-attention, MDTA-style — the channel branch
    of DAT's dual aggregation. Attention is computed across channels (C x C),
    so cost is independent of spatial token count."""
...
class DATBlock(nn.Module):
    """Dual aggregation block: spatial self-attention + channel self-attention."""
...
class DualAggregationGroup(nn.Module):
    def __init__(self, dim=96, heads=4, depth=4):
        super().__init__()
        self.blocks = nn.Sequential(
            *[DATBlock(dim=dim, heads=heads) for _ in range(depth)]
        )
        self.conv = nn.Conv2d(dim, dim, kernel_size=3, padding=1)
...
class DAT(nn.Module):
    def __init__(self, scale, channels=5, dim=96, n_groups=4, depth=4, heads=4):
        ...
        self.body = nn.Sequential(
            *[DualAggregationGroup(dim=dim, heads=heads, depth=depth)
              for _ in range(n_groups)]
        )
...
# ============================================================
# REVISION (B5): MambaIR - state-space image restoration
...
# (head -> residual state-space groups -> body_conv -> global residual ->
#  pixel-shuffle upsampler -> tail -> clamp).
#
# MambaIR replaces the self-attention token mixer used by SwinIR/HAT/DAT with a
# Vision State-Space Module (VSSM / SS2D): the 2-D feature map is unrolled into
# a 1-D sequence, the sequence is processed by a *selective* (input-dependent)
# diagonal state-space model, and several scan directions are merged. Because a
# state-space scan is linear in sequence length, MambaIR-based SR is the only
# long-range model here that does not need a reduced inference tile. A local
# channel-attention branch (CAB) runs in parallel, as in the paper, to restore
# the local inductive bias that the sequential scan gives up.
#
# Deliberate simplifications, so the model stays inside the same compact and
# equalized budget as the other baselines:
#   * the hardware-aware "selective scan" CUDA kernel of the official release is
#     replaced by a numerically stable chunked scan written in plain PyTorch;
#   * two scan directions (forward / backward over the row-major unrolling) are
#     used instead of four;
#   * d_state and the expansion ratio are reduced.
# It is therefore referred to as MambaIR-based SR, exactly as the SwinIR-based,
# HAT-based and DAT-based baselines are.
# ============================================================

# Tunable from the shell so the memory/'speed point can be chosen without editing
# this file:  SR_MAMBA_DIM / _GROUPS / _DEPTH / _D_STATE / _EXPAND.
MAMBA_DIM = int(os.environ.get("SR_MAMBA_DIM", "96"))
MAMBA_GROUPS = int(os.environ.get("SR_MAMBA_GROUPS", "4"))
MAMBA_DEPTH = int(os.environ.get("SR_MAMBA_DEPTH", "5"))
# d_state drives the (B, D, L, N) activation tensors, which dominate both peak
# memory and step time. Halving it roughly halves both and barely changes the
# parameter count, so it is the first knob to turn if x2 does not fit.
# Set to 2 after profiling on the target GPU: at HR_PATCH_SIZE=192 the x2 case
# peaks at 53 GiB with d_state=2 versus 88 GiB with d_state=4, which does not
# leave room for the composite loss on a 95 GiB card. The parameter count is
# essentially unchanged (2.88M vs 2.91M at x4, against 2.93M for DAT), so the
# equalized-budget comparison is unaffected. THIS MUST BE THE SAME FOR EVERY
# SCALE - a mismatch would mean different models across x2/x3/x4.
MAMBA_D_STATE = int(os.environ.get("SR_MAMBA_D_STATE", "2"))
MAMBA_EXPAND = int(os.environ.get("SR_MAMBA_EXPAND", "1"))   # d_inner = EXPAND * dim
MAMBA_SCAN_CHUNK = 64    # chunk length of the stable associative scan
MAMBA_LOGDECAY_FLOOR = -24.0   # safety clamp on the WITHIN-CHUNK cumulative log-decay
MAMBA_LOG_A_FLOOR = -0.35      # per-step log-decay floor (retention >= exp(-0.35) = 0.70)
# Invariant that keeps the scan exact:  CHUNK * |LOG_A_FLOOR| < |LOGDECAY_FLOOR|
# 64 * 0.35 = 22.4 < 24, so the within-chunk clamp never binds and the chunked
# scan is an exact solver; the clamp only exists to keep exp() finite if the
# per-step floor is ever relaxed. Raising CHUNK reduces the number of sequential
# steps (faster) but requires tightening LOG_A_FLOOR to preserve the invariant.
assert MAMBA_SCAN_CHUNK * abs(MAMBA_LOG_A_FLOOR) < abs(MAMBA_LOGDECAY_FLOOR), \
    "MambaIR scan: chunk x per-step log-decay floor must stay inside the cumulative clamp"


def _selective_scan_chunked(bu, log_a, chunk=MAMBA_SCAN_CHUNK):
    """Numerically stable replacement for the selective-scan CUDA kernel.

    Solves the diagonal linear recurrence
        h_t = exp(log_a_t) * h_{t-1} + bu_t
    for tensors shaped (B, D, L, N).

    The naive parallel form  h_t = exp(S_t) * cumsum(exp(-S_s) * bu_s)  with
    S = cumsum(log_a) overflows for long sequences, because exp(-S_s) grows
    without bound. The sequence is therefore split into short chunks and only
    the chunk-boundary state is carried forward sequentially. Cost is O(L) and
    only L/chunk sequential steps remain.

    IMPORTANT: the clamp on the cumulative log-decay must never bind. If it did,
    a pair (s, t) whose true weight exp(S_t - S_s) is ~0 would instead be given
    weight exp(clamp) * exp(-clamp) = 1, i.e. a fully decayed state would be
    resurrected. The caller therefore floors the per-step log_a so that
    chunk * |floor| stays inside the cumulative clamp (see MAMBA_LOG_A_FLOOR).
    """
    b, d, l, n = bu.shape
    pad = (chunk - (l % chunk)) % chunk
    if pad:
        bu = F.pad(bu, (0, 0, 0, pad))
        log_a = F.pad(log_a, (0, 0, 0, pad), value=0.0)

    nc = bu.shape[2] // chunk
    bu = bu.view(b, d, nc, chunk, n)
    log_a = log_a.view(b, d, nc, chunk, n)

    s = torch.cumsum(log_a, dim=3).clamp(min=MAMBA_LOGDECAY_FLOOR)
    decay = torch.exp(s)                     # (0, 1]
    inv_decay = torch.exp(-s)                # bounded by exp(-floor)

    local = decay * torch.cumsum(inv_decay * bu, dim=3)

    outs = []
    carry = bu.new_zeros(bu.shape[0], d, n)
    for c in range(nc):
        hc = local[:, :, c] + decay[:, :, c] * carry.unsqueeze(2)
        outs.append(hc)
        carry = hc[:, :, -1]
    h = torch.cat(outs, dim=2)
...
class SS2D(nn.Module):
    """Vision State-Space Module: selective diagonal SSM over 2-D features."""

    def __init__(self, dim=96, d_state=MAMBA_D_STATE, expand=MAMBA_EXPAND,
                 n_directions=2):
        super().__init__()
        ...
        # Per-direction projections producing the input-dependent dt, B and C.
        ...
        # A is kept negative and diagonal via A = -exp(A_log), as in Mamba.
        a = torch.arange(1, d_state + 1, dtype=torch.float32)
        a = a.view(1, 1, d_state).repeat(self.n_dir, self.d_inner, 1)
        self.A_log = nn.Parameter(torch.log(a))
        self.D = nn.Parameter(torch.ones(self.n_dir, self.d_inner))

        self.out_norm = nn.LayerNorm(self.d_inner)
        self.out_proj = nn.Linear(self.d_inner, dim, bias=False)

    def _prepare(self, x, direction):
        """Build the (parameter-dependent) scan inputs for one direction.

        Returns bu, log_a  -> (B, D, L, N)  and  cmat -> (B, L, N).
        Kept separate from the scan itself so that the scans of all directions
        can be merged into a single batched call (see forward): the recurrence
        solver is parameter-free, so stacking directions along the batch axis
        halves the number of sequential chunk steps, which is what dominates
        the runtime at the patch sizes used here.
        """
        b, d, l = x.shape

        proj = self.x_proj[direction](x.transpose(1, 2))           # (B, L, R+2N)
        dt, bmat, cmat = torch.split(
            proj, [self.dt_rank, self.d_state, self.d_state], dim=-1
        )
        dt = F.softplus(self.dt_proj[direction](dt))               # (B, L, D)
        dt = dt.transpose(1, 2)                                    # (B, D, L)

        a_neg = -torch.exp(self.A_log[direction].float())          # (D, N)
        log_a = dt.unsqueeze(-1) * a_neg.unsqueeze(0).unsqueeze(2)  # (B,D,L,N)
        # Floor the per-step log-decay so the chunked scan stays exact (see the
        # invariant next to MAMBA_LOG_A_FLOOR). Retention per step is therefore
        # at least exp(MAMBA_LOG_A_FLOOR); over one chunk the state can still
        # decay by exp(-22.4) ~ 2e-10, so the usable memory range is unaffected.
        log_a = log_a.clamp(min=MAMBA_LOG_A_FLOOR)

        bu = (dt * x).unsqueeze(-1) * bmat.unsqueeze(1)            # (B,D,L,N)
        return bu, log_a, cmat
    ...
    def forward(self, x):
        ...
        xs = xs.transpose(1, 2).reshape(b, self.d_inner, h, w)
        xs = self.act(self.conv2d(xs))
        seq = xs.flatten(2)                                        # (B, D, L)

        # The selective scan is accumulated in float32 for numerical stability
        # even when the surrounding model runs under automatic mixed precision.
        with torch.cuda.amp.autocast(enabled=False):
            seq32 = seq.float()
            nb = seq32.shape[0]

            # Per-direction projections (cheap, parameter-dependent) ...
            prepared = []
            for k in range(self.n_dir):
                src = seq32 if k == 0 else seq32.flip(-1)
                prepared.append(self._prepare(src, k))

            # ... then ONE batched recurrence for every direction. The solver is
            # parameter-free, so the directions can share a single chunk loop.
            bu = torch.cat([p[0] for p in prepared], dim=0)
            log_a = torch.cat([p[1] for p in prepared], dim=0)
            # NB: do not name this `h` - `h` is the spatial height unpacked above.
            states = _selective_scan_chunked(bu, log_a)

            out = None
            for k in range(self.n_dir):
                hk = states[k * nb:(k + 1) * nb]
                cmat = prepared[k][2]
                src = seq32 if k == 0 else seq32.flip(-1)
                yk = (hk * cmat.unsqueeze(1)).sum(dim=-1)
                yk = yk + self.D[k].view(1, -1, 1) * src
                if k > 0:
                    yk = yk.flip(-1)
                out = yk if out is None else out + yk
            out = out / float(self.n_dir)
        out = out.to(seq.dtype)

        out = self.out_norm(out.transpose(1, 2))                   # (B, L, D)
        out = out * F.silu(z)
...
class MambaCAB(nn.Module):
    """Local channel-attention branch used alongside the state-space mixer."""

    def __init__(self, dim=96, compress=3):
        super().__init__()
        hidden = max(dim // compress, 8)
        self.body = nn.Sequential(
...
class RSSB(nn.Module):
    """Residual State-Space Block: VSSM branch + channel-attention branch."""
    ...
    def __init__(self, dim=96, d_state=MAMBA_D_STATE, expand=MAMBA_EXPAND,
        ...
        self.norm1 = nn.LayerNorm(dim)
        self.ssm = SS2D(dim=dim, d_state=d_state, expand=expand)

        self.norm2 = nn.LayerNorm(dim)
        self.cab = MambaCAB(dim)
        self.ca_weight = ca_weight

        # Learnable residual scale, as in the MambaIR residual state-space block.
        self.skip_scale1 = nn.Parameter(torch.ones(1))
        self.skip_scale2 = nn.Parameter(torch.ones(1))

    def _norm2d(self, norm, x):
        b, c, h, w = x.shape
        y = norm(x.flatten(2).transpose(1, 2))
        return y.transpose(1, 2).reshape(b, c, h, w)

    def forward(self, x):
        x = x * self.skip_scale1 + self.ssm(self._norm2d(self.norm1, x))
        x = x * self.skip_scale2 + self.ca_weight * self.cab(self._norm2d(self.norm2, x))
        return x


class ResidualStateSpaceGroup(nn.Module):
    def __init__(self, dim=96, depth=MAMBA_DEPTH):
        super().__init__()
        self.blocks = nn.Sequential(*[RSSB(dim=dim) for _ in range(depth)])
        self.conv = nn.Conv2d(dim, dim, kernel_size=3, padding=1)
...
class MambaIR(nn.Module):
    def __init__(self, scale, channels=5, dim=MAMBA_DIM,
                 n_groups=MAMBA_GROUPS, depth=MAMBA_DEPTH):
        super().__init__()
        ...
        self.body = nn.Sequential(
            *[ResidualStateSpaceGroup(dim=dim, depth=depth)
              for _ in range(n_groups)]
        )
...
def build_model(method, scale, channels=5):
    ...
    # REVISION (A3): parameter-matched widened joint EDSR.
    # Widened nf so the (joint) total parameter count approaches the band-wise
    # total (~5x the base joint EDSR). Verify with count_model_parameters at
    # build time and adjust EDSR_WIDE_NF if needed to match the band-wise total.
    if method == "edsr_wide":
        return EDSR(scale=scale, channels=channels,
                    nf=EDSR_WIDE_NF, n_blocks=EDSR_WIDE_NBLOCKS)
    ...
    # REVISION (B4): HAT recent SOTA baseline (compact, matched budget).
    if method == "hat":
        return HAT(scale=scale, channels=channels,
                   dim=96, n_groups=4, depth=4, heads=4)

    # REVISION (B4b): DAT recent SOTA baseline (compact, matched budget).
    if method == "dat":
        return DAT(scale=scale, channels=channels,
                   dim=96, n_groups=4, depth=4, heads=4)

    # REVISION (B5): MambaIR recent SOTA baseline (compact, matched budget).
    if method == "mambair":
        return MambaIR(scale=scale, channels=channels,
                       dim=MAMBA_DIM, n_groups=MAMBA_GROUPS, depth=MAMBA_DEPTH)
...
def esrgan_stable_index_loss(pred, target, pix_mask=None, beta=0.05):
    diff = torch.abs(pred - target)
    loss = torch.where(diff < beta, 0.5 * diff * diff / beta, diff - 0.5 * beta)

    if pix_mask is not None:
        loss = loss * pix_mask
        return loss.sum() / (pix_mask.sum() + 1e-8)

    return loss.mean()


def esrgan_stable_vi_loss(sr, hr, mask=None):
    def safe_indices(x):
        green = x[:, 1:2, :, :]
        red = x[:, 2:3, :, :]
        rededge = x[:, 3:4, :, :]
        ...
        def safe_ratio(num, a, b):
            denom = (torch.abs(a) + torch.abs(b)).clamp_min(float(ESRGAN_STABLE_VI_DENOM_EPS))
            return torch.clamp(num / denom, -1.0, 1.0)

        ndvi = safe_ratio(nir - red, nir, red)
        gndvi = safe_ratio(nir - green, nir, green)
        ndre = safe_ratio(nir - rededge, nir, rededge)
        return ndvi, gndvi, ndre

    sr_ndvi, sr_gndvi, sr_ndre = safe_indices(sr)
    hr_ndvi, hr_gndvi, hr_ndre = safe_indices(hr)

    pix_mask = None
    if mask is not None and USE_MASKED_LOSS:
        pix_mask = (mask.mean(dim=1, keepdim=True) > 0.5).float()

    beta = float(ESRGAN_STABLE_VI_BETA)
    loss = (
        esrgan_stable_index_loss(sr_ndvi, hr_ndvi, pix_mask, beta=beta) +
        esrgan_stable_index_loss(sr_gndvi, hr_gndvi, pix_mask, beta=beta) +
        esrgan_stable_index_loss(sr_ndre, hr_ndre, pix_mask, beta=beta)
    ) / 3.0

    return loss


def esrgan_l1_sam_vi_ramp(epoch):
    if epoch < ESRGAN_L1_SAM_VI_START_EPOCH:
        return 0.0
    progress = float(epoch - ESRGAN_L1_SAM_VI_START_EPOCH + 1)
    return min(1.0, max(0.0, progress / max(float(ESRGAN_L1_SAM_VI_RAMP_EPOCHS), 1.0)))


def esrgan_ablation_content_loss(sr, hr, mask=None, channels=5, epoch=1):
    """Stable content loss for ESRGAN loss-ablation profiles.

    The default ESRGAN path is unchanged. For L1/SAM/VI ablations, SAM and VI
    are weighted lower than in CNN models and ramped in gradually so the
    generator first learns reflectance reconstruction before spectral penalties
    dominate.
    """
    profile_id = CURRENT_LOSS_PROFILE_ID
    ...
    if profile_id == DEFAULT_LOSS_PROFILE_ID:
        ...
        if channels == 5:
            ...
            sam = torch.tensor(0.0, device=sr.device)
            vi = torch.tensor(0.0, device=sr.device)
    ...
    ramp = min(1.0, max(0.0, float(epoch) / max(float(ESRGAN_ABLATION_LOSS_RAMP_EPOCHS), 1.0)))

    l1 = masked_l1_loss(sr, hr, mask)
    content_loss = l1

    if channels == 5 and profile_id in ("l1_sam", "l1_sam_vi"):
        content_loss = content_loss + (ESRGAN_ABLATION_SAM_WEIGHT * ramp) * sam_loss(sr, hr, mask)

    if channels == 5 and profile_id == "l1_sam_vi":
        vi_ramp = esrgan_l1_sam_vi_ramp(epoch)
        content_loss = content_loss + (
            ESRGAN_L1_SAM_VI_WEIGHT * vi_ramp
        ) * esrgan_stable_vi_loss(sr, hr, mask)

    return torch.clamp(content_loss, max=float(ESRGAN_ABLATION_CONTENT_CLAMP))


def get_esrgan_adv_settings(epoch):
    if CURRENT_LOSS_PROFILE_ID == DEFAULT_LOSS_PROFILE_ID:
        return epoch > ESRGAN_PRETRAIN_EPOCHS, ESRGAN_ADV_WEIGHT
    if CURRENT_LOSS_PROFILE_ID == "l1_sam_vi":
        return epoch > ESRGAN_L1_SAM_VI_PRETRAIN_EPOCHS, ESRGAN_L1_SAM_VI_ADV_WEIGHT
    return epoch > ESRGAN_ABLATION_PRETRAIN_EPOCHS, ESRGAN_ABLATION_ADV_WEIGHT
...
def calc_psnr_standard(hr, sr, data_range=1.0):
    """REVISION (D1, R3#5): standard PSNR using a fixed peak value over the full
    reflectance range [0, 1] (data_range=1.0), for comparability with the SR
    literature. Reported alongside the scene-dependent 99.5th-percentile PSNR."""
    hr, sr, valid = finite_pair(hr, sr)
...
def vegetation_indices_np(cube, clip01=False):
    # REVISION (D3, R3#9): optionally clip reflectance to the valid [0, 1] range
    # before computing indices. The deployed pipeline bounds model output to
    # [0, 1.5] (torch.clamp) and computes VIs directly (clip01=False); the
    # clipped variant (clip01=True) is reported alongside for transparency.
    if clip01:
...
def calc_vi_errors(hr, sr):
    ...
    if hr.shape[-1] < 5 or sr.shape[-1] < 5:
        return {
            ...
            "GNDVI_MAE": np.nan,
            "NDRE_MAE": np.nan,
            "NDVI_MAE_clip01": np.nan,
            "GNDVI_MAE_clip01": np.nan,
            "NDRE_MAE_clip01": np.nan,
        }
    ...
    # REVISION (D3): VI errors with reflectance clipped to [0, 1] before index
    # computation (HR is already in-range; this mainly affects SR outputs).
    if REPORT_VI_CLIP01:
        hr_ndvi_c, hr_gndvi_c, hr_ndre_c = vegetation_indices_np(hr, clip01=True)
        sr_ndvi_c, sr_gndvi_c, sr_ndre_c = vegetation_indices_np(sr, clip01=True)
        result["NDVI_MAE_clip01"] = float(np.nanmean(np.abs(hr_ndvi_c - sr_ndvi_c)))
        result["GNDVI_MAE_clip01"] = float(np.nanmean(np.abs(hr_gndvi_c - sr_gndvi_c)))
        result["NDRE_MAE_clip01"] = float(np.nanmean(np.abs(hr_ndre_c - sr_ndre_c)))
...
def calc_metrics_sequential(hr, sr, scale=None):
    metrics = {
        "MSE": calc_mse(hr, sr),
        "RMSE": calc_rmse(hr, sr),
        "MAE": calc_mae(hr, sr),
        "PSNR": calc_psnr(hr, sr),
        # REVISION (D1, R3#5): standard PSNR over the full [0,1] reflectance range.
        "PSNR_std": calc_psnr_standard(hr, sr, data_range=1.0),
...
def calc_metrics(hr, sr, scale=None):
    ...
    EVAL_METRIC_PARALLEL computes independent CPU-heavy metric groups with a
    small thread pool. GPU inference remains outside this function and serial.
    """
    if not EVAL_METRIC_PARALLEL or int(EVAL_METRIC_CPU_WORKERS) <= 1:
        return calc_metrics_sequential(hr, sr, scale=scale)

    workers = max(1, int(EVAL_METRIC_CPU_WORKERS))
    metrics = {}
    ...
    def basic_metrics():
        return {
            ...
            "MAE": calc_mae(hr, sr),
            "PSNR": calc_psnr(hr, sr),
            # REVISION (D1): standard PSNR over full [0,1] range.
            "PSNR_std": calc_psnr_standard(hr, sr, data_range=1.0),
            "R2": calc_r2(hr, sr),
        }
    ...
    def spectral_metrics():
        return {
            ...
            "UQI": calc_uqi(hr, sr),
            "RASE": calc_rase(hr, sr),
            "ERGAS": calc_ergas(hr, sr, scale) if scale is not None else np.nan,
        }

    def optional_sharpness_metrics():
        if COMPUTE_SHARPNESS_METRICS:
            return calc_sharpness_metrics(hr, sr)
        return {
            "Gradient_MAE": np.nan,
            "Laplacian_MAE": np.nan,
            "Tenengrad_HR": np.nan,
    ...
    def bandwise_ssim_metrics():
        if COMPUTE_BANDWISE_SSIM:
            return calc_bandwise_ssim(hr, sr)
        return {f"SSIM_{name}": np.nan for name in BAND_NAMES}

    tasks = [
        basic_metrics,
        spectral_metrics,
        optional_sharpness_metrics,
        lambda: calc_bandwise_rmse(hr, sr),
        lambda: calc_bandwise_mae(hr, sr),
        lambda: calc_bandwise_psnr(hr, sr),
        bandwise_ssim_metrics,
        lambda: calc_vi_errors(hr, sr),
    ]
...
def infer_full_image(model, lr_cube, scale, norm_factor, channels=5, band_index=None, tile_lr_size=None, tile_overlap=None):
    ...
    for y in y_list:
        for x in x_list:
            ...
            if key in seen:
                continue
            seen.add(key)
            tile_jobs.append(key)

    tile_batch_size = int(max(1, EVAL_TILE_BATCH_SIZE))

    def add_prediction(y1, y2, x1, x2, pred_np):
    ...
    for start_idx in range(0, len(tile_jobs), tile_batch_size):
        batch_jobs = tile_jobs[start_idx:start_idx + tile_batch_size]
        tiles = [lr_norm[y1:y2, x1:x2, :] for y1, y2, x1, x2 in batch_jobs]

        try:
            inp = torch.stack([np_to_tensor(tile) for tile in tiles], dim=0).to(DEVICE)
            pred_batch = model(inp)

            for (y1, y2, x1, x2), pred in zip(batch_jobs, pred_batch):
                add_prediction(y1, y2, x1, x2, tensor_to_np(pred))

        except RuntimeError as e:
            if "out of memory" not in str(e).lower() or tile_batch_size <= 1:
                raise

            print(
                f"[WARN] Tile batch inference OOM with batch={tile_batch_size}. "
                "Fallback to per-tile inference for this image."
            )
            if torch.cuda.is_available():
                torch.cuda.empty_cache()

            for y1, y2, x1, x2 in batch_jobs:
...
def get_checkpoint_path(mode, method, scale, band_name=None):
    ...
    # REVISION: ensure the checkpoint directory exists before save/load. New
    # models (hat, dat, edsr_wide) are trained from scratch and their parent
    # folder may not pre-exist on the server.
    try:
        Path(checkpoint_root).mkdir(parents=True, exist_ok=True)
    except Exception as _e:
        print(f"[WARN] could not create checkpoint dir {checkpoint_root}: {_e}")
...
def get_checkpoint_run_name(path):
    try:
        # .../<run_name>/sr_checkpoints/<checkpoint>.pth
        return Path(path).parent.parent.name
    except Exception:
        return ""


def checkpoint_preference_rank(path):
    run_name = get_checkpoint_run_name(path)
    try:
        return CHECKPOINT_SEARCH_PREFER_RUNS.index(run_name)
    except ValueError:
        return len(CHECKPOINT_SEARCH_PREFER_RUNS)


def resolve_checkpoint_path_for_load(expected_path):
    expected_path = Path(expected_path)
    if expected_path.exists():
        return expected_path

    if not AUTO_FIND_CHECKPOINTS:
        return None

    search_root = Path(CHECKPOINT_SEARCH_ROOT)
    if not search_root.exists():
        return None

    ckpt_name = expected_path.name
    candidates = [p for p in search_root.glob(f"*/sr_checkpoints/{ckpt_name}") if p.is_file()]
    ...
    candidates = sorted(
        candidates,
        key=lambda p: (checkpoint_preference_rank(p), -p.stat().st_mtime, str(p)),
    )
    selected = candidates[0]

    if len(candidates) > 1:
        shown = "; ".join([f"{get_checkpoint_run_name(p)}:{p.stat().st_mtime:.0f}" for p in candidates[:5]])
        print(f"[INFO] Multiple checkpoint candidates for {ckpt_name}. Selected {selected}. Candidates={shown}")
    else:
        print(f"[INFO] Auto-found checkpoint for {ckpt_name}: {selected}")

    return selected


def load_trained_model_from_checkpoint(method, scale, mode="joint5ch", channels=5, band_index=None, band_name=None):
    expected_ckpt_path = get_checkpoint_path(mode=mode, method=method, scale=scale, band_name=band_name)
    ckpt_path = resolve_checkpoint_path_for_load(expected_ckpt_path)
...
def should_skip_eval(mode, method, scale, expected_rows=None):
    case_method = get_case_method_name(method)
    csv_path = OUTPUT_DIR / "sr_logs" / f"metrics_{mode}_{case_method}_x{scale}.csv"
    if not SKIP_EVAL_IF_METRICS_EXISTS or not csv_path.exists():
        return False
    ...
    try:
        existing_rows = len(pd.read_csv(csv_path))
    except Exception as e:
        print(f"[WARN] Existing metrics CSV could not be checked, rerun evaluation: {csv_path} | {e}")
        return False

    if existing_rows >= int(expected_rows):
        return True

    print(
        f"[INFO] Existing metrics CSV has {existing_rows} rows but current evaluation needs "
        f"{int(expected_rows)} rows. Rerun evaluation: {csv_path}"
    )
    return False
...
def train_esrgan(
    ...
    for epoch in range(start_epoch + 1, EPOCHS + 1):
        ...
        use_gan, current_adv_weight = get_esrgan_adv_settings(epoch)
        ...
        for lr_patch, hr_patch, mask_patch in train_loader:
            ...
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                content_loss = esrgan_ablation_content_loss(
                    sr_patch,
                    hr_patch,
                    mask_patch,
                    channels=channels,
                ...
                if use_gan:
                    ...
                    g_loss = content_loss + current_adv_weight * adv
                else:
                    g_loss = content_loss
        ...
        with torch.no_grad():
            for lr_patch, hr_patch, mask_patch in val_loader:
                ...
                with torch.cuda.amp.autocast(enabled=amp_enabled):
                    sr_patch = generator(lr_patch)
                    loss = esrgan_ablation_content_loss(
                        sr_patch,
                        hr_patch,
                        mask_patch,
                        channels=channels,
        ...
        history.append({
            ...
            "val_loss": val_loss,
            "use_gan": int(use_gan),
            "adv_weight": current_adv_weight if use_gan else 0.0,
            "esrgan_ablation_sam_weight": ESRGAN_ABLATION_SAM_WEIGHT,
            "esrgan_ablation_vi_weight": ESRGAN_ABLATION_VI_WEIGHT,
            "esrgan_l1_sam_vi_weight": ESRGAN_L1_SAM_VI_WEIGHT,
            "esrgan_l1_sam_vi_ramp": esrgan_l1_sam_vi_ramp(epoch) if loss_profile_id == "l1_sam_vi" else 0.0,
            "train_time_sec": train_time_sec,
            "val_time_sec": val_time_sec,
            "epoch_time_sec": epoch_time_sec,
            "train_time_hms": format_seconds(train_time_sec),
        ...
        print(
            ...
            f"Val={val_loss:.6f} | "
            f"GAN={use_gan} | "
            f"AdvW={(current_adv_weight if use_gan else 0.0):.1e} | "
            f"Time={format_seconds(epoch_time_sec)}"
        )
...
def evaluate_joint5ch(method, scale, model, test_records):
    ...
    for idx, record in enumerate(test_records):
        ...
        try:
            ...
            hr = load_npy(hr_path)
            # REVISION (B2): use blur+bicubic LR at eval when that mode is active.
            lr = load_lr_cube(record, scale, hr_cube=hr)
...
def make_summary_rows_for_methods(all_rows, metric_keys, method_cases, mode_filter=None):
    summary_rows = []
        ...
        for mode in EXPERIMENT_MODES:
            ...
            for case in method_cases:
                ...
                summary_row = {
                    "Scale": f"x{scale}",
                    "Mode": mode,
                    "Base_Method": case.get("base_method", method_name),
                    "Loss_Profile_ID": case.get("loss_profile_id", DEFAULT_LOSS_PROFILE_ID),
                    "Loss_Profile": case.get("loss_profile_label", get_loss_profile_label(case.get("loss_profile_id", DEFAULT_LOSS_PROFILE_ID))),
                    "Method": method_name,
                    "N_scenes": int(len(rows)),
                }
                ...
                for k in metric_keys:
                    ...
                    if len(vals) > 0:
                        ...
                        summary_row[f"{k}_median"] = float(np.median(vals))
                        summary_row[f"{k}_min"] = float(np.min(vals))
                        summary_row[f"{k}_max"] = float(np.max(vals))
                    else:
                        summary_row[f"{k}_mean"] = ""
                        summary_row[f"{k}_std"] = ""
                        summary_row[f"{k}_median"] = ""
                        summary_row[f"{k}_min"] = ""
                        summary_row[f"{k}_max"] = ""
...
def get_loss_ablation_method_cases():
    method_cases = []
    for base_method in LOSS_ABLATION_METHODS:
        if base_method == "bicubic":
            continue
        profile_ids = []
        if LOSS_ABLATION_INCLUDE_FULL_LOSS_BASELINE:
            profile_ids.append(DEFAULT_LOSS_PROFILE_ID)
        profile_ids.extend(LOSS_ABLATION_PROFILE_IDS)
        ...
        for profile_id in profile_ids:
            ...
            method_cases.append({
                ...
                "loss_profile_id": profile_id,
                "loss_profile_label": get_loss_profile_label(profile_id),
                "method": get_case_method_name(base_method, profile_id),
            })
    return method_cases
...
def save_total_metrics(all_rows):
    ...
    if RUN_LOSS_ABLATION:
        loss_ablation_rows = make_summary_rows_for_methods(
            all_rows=all_rows,
            metric_keys=table_metrics,
            method_cases=get_loss_ablation_method_cases(),
            mode_filter="joint5ch",
        )
        loss_ablation_csv = OUTPUT_DIR / "sr_logs" / f"summary_table_loss_ablation_joint5ch_{scale_tag}.csv"
        write_dict_csv(loss_ablation_csv, loss_ablation_rows)
    ...
    if SAVE_EXCEL_SUMMARY:
        try:
            save_excel_summary(
                ...
                loss_ablation_rows=loss_ablation_rows,
            )
        except Exception as e:
...
def save_excel_summary(all_rows, paper_rows, loss_ablation_rows=None):
    ...
    loss_ablation_df = pd.DataFrame(loss_ablation_rows or [])
    ...
    if not loss_ablation_df.empty:
        for col in loss_ablation_df.columns:
            if col.endswith(("_mean", "_std", "_median", "_min", "_max")) or col == "N_scenes":
                loss_ablation_df[col] = pd.to_numeric(loss_ablation_df[col], errors="coerce")
    ...
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        ...
        loss_ablation_df.to_excel(writer, sheet_name="Loss_Ablation", index=False)
        ...
        for name in [
            ...
            "Runtime", "Train_history_summary", "Model_Complexity_Time", "Summary_mean_std_raw", "Loss_Ablation"
        ]:
        ...
        for name in [
            ...
            "Bandwise_compare", "Scale_compare", "Runtime", "Train_history_summary", "Loss_Ablation"
        ]:
            if name in wb.sheetnames:
                ws = wb[name]
                for col in ["A", "B", "C"]:
...
def report_excluded_scenes(all_metric_rows, rmse_threshold=1.0):
    """REVISION (D2, R3#8): report the count and percentage of test scenes with
    RMSE >= threshold (the pairwise-exclusion rule used in the statistical
    analysis), grouped by scale/mode/method, plus the mean valid-pixel ratio of
    the excluded scenes. Written to a dedicated CSV."""
    if not all_metric_rows:
        return

    groups = {}
    for r in all_metric_rows:
        key = (r.get("scale"), r.get("mode"), r.get("method"))
        g = groups.setdefault(key, {"total": 0, "excluded": 0, "excl_valid_ratios": []})
        g["total"] += 1
        rmse = r.get("RMSE", np.nan)
        if rmse is not None and np.isfinite(rmse) and rmse >= rmse_threshold:
            g["excluded"] += 1
            vr = r.get("valid_metric_ratio", np.nan)
            if vr is not None and np.isfinite(vr):
                g["excl_valid_ratios"].append(float(vr))

    rows = []
    for (scale, mode, method), g in sorted(groups.items(), key=lambda x: str(x[0])):
        total = g["total"]
        excluded = g["excluded"]
        pct = (100.0 * excluded / total) if total > 0 else np.nan
        mean_vr = float(np.nanmean(g["excl_valid_ratios"])) if g["excl_valid_ratios"] else np.nan
        rows.append({
            "scale": scale,
            "mode": mode,
            "method": method,
            ...
            "n_excluded": excluded,
            "pct_excluded": pct,
            "mean_valid_ratio_excluded": mean_vr,
        })

    out_csv = OUTPUT_DIR / "sr_logs" / "excluded_scenes_report.csv"
    write_dict_csv(out_csv, rows)
    print(f"[D2] Saved excluded-scene report (RMSE >= {rmse_threshold}): {out_csv}")


def run_revision_noise_experiment(test_records):
    """REVISION (B3, R3#6): noise-robustness evaluation. Adds Gaussian noise at
    the configured sigmas to the (area) LR test inputs and evaluates the given
    representative methods at the given scales WITHOUT retraining, using the
    existing area-trained joint five-channel checkpoints. Writes one CSV per
    (method, scale) plus a combined summary CSV. Isolated from the main metrics
    so it never affects the primary tables."""
    active_sigmas = [float(s) for s in EVAL_NOISE_SIGMAS if float(s) > 0.0]
    if len(active_sigmas) == 0:
        return

    print("\n####################################")
    print(f"REVISION B3: noise-robustness eval, sigmas={active_sigmas}")
    print("####################################")
    ...
    for scale in EVAL_NOISE_SCALES:
        for method in EVAL_NOISE_METHODS:
            model = load_trained_model_from_checkpoint(
                method=method, scale=scale, mode=mode, channels=5
            )
            if model is None and method != "bicubic":
                print(f"[WARN] B3: checkpoint not found, skip {method} x{scale}")
                continue
            ...
            for sigma in active_sigmas:
                ...
                for idx, record in enumerate(test_records):
                    scene_id = record["scene_id"]
                    try:
                        hr = load_npy(get_hr_path(record))
                        lr = load_lr_cube(record, scale, hr_cube=hr)

                        # Additive Gaussian noise on the LR input, then clamp to
                        # the valid reflectance range used by the models [0, 1.5].
                        noise = rng.normal(0.0, sigma, size=lr.shape).astype(np.float32)
                        lr_noisy = np.clip(lr + noise, 0.0, 1.5).astype(np.float32)

                        hr = align_hr_to_lr_scale(hr, lr_noisy, scale)
                        norm_factor = robust_norm_factor(hr)

                        if method == "bicubic":
                            sr = bicubic_sr(lr_noisy, hr.shape)
                        else:
                            sr = infer_full_image(
                                model=model, lr_cube=lr_noisy, scale=scale,
                                norm_factor=norm_factor, channels=5,
                                tile_lr_size=infer_tile_size,
                                tile_overlap=infer_tile_overlap,
                            )
                            if sr.shape[0] != hr.shape[0] or sr.shape[1] != hr.shape[1]:
                        ...
                        sr = np.nan_to_num(sr, nan=0.0, posinf=0.0, neginf=0.0)
                        hr_m, sr_m, _, vr = mask_nodata_for_metrics(hr, sr)
                        metrics = calc_metrics(hr_m, sr_m, scale=scale)
                        ...
                        print(f"[ERROR] B3 eval failed scene={scene_id} {method} x{scale} sigma={sigma}: {e}")

                out_csv = OUTPUT_DIR / "sr_logs" / f"metrics_noise_{method}_x{scale}_sigma{sigma}.csv"
                write_dict_csv(out_csv, rows)
                if len(rows) > 0:
                    mean_rmse = float(np.nanmean([r["RMSE"] for r in rows]))
                    mean_sam = float(np.nanmean([r["SAM_degree"] for r in rows]))
                    print(f"[B3] {method} x{scale} sigma={sigma}: "
                          f"mean RMSE={mean_rmse:.6f}, mean SAM={mean_sam:.4f}, n={len(rows)}")

    if len(combined_rows) > 0:
        summary_csv = OUTPUT_DIR / "sr_logs" / "metrics_noise_ALL.csv"
        write_dict_csv(summary_csv, combined_rows)
        print(f"[B3] Saved combined noise metrics: {summary_csv}")
...
def main():
    ...
    try:
        ...
        if hasattr(torch, "set_num_interop_threads"):
            torch.set_num_interop_threads(max(1, min(4, int(TORCH_CPU_THREADS))))
        cv2.setNumThreads(max(1, int(TORCH_CPU_THREADS)))
        print(
            f"[INFO] CPU threads: torch={TORCH_CPU_THREADS}, "
            f"cv2={TORCH_CPU_THREADS}, metric_workers={EVAL_METRIC_CPU_WORKERS}, "
            f"metric_parallel={EVAL_METRIC_PARALLEL}, tile_batch={EVAL_TILE_BATCH_SIZE}"
        )
    except Exception as e:
        print(f"[WARN] Failed to configure CPU threads: {e}")
    ...
    run_config = {
        ...
        "AUTO_FIND_CHECKPOINTS": AUTO_FIND_CHECKPOINTS,
        "CHECKPOINT_SEARCH_ROOT": str(CHECKPOINT_SEARCH_ROOT),
        "CHECKPOINT_SEARCH_PREFER_RUNS": CHECKPOINT_SEARCH_PREFER_RUNS,
        ...
        "VI_WEIGHT": VI_WEIGHT,
        "ESRGAN_PRETRAIN_EPOCHS": ESRGAN_PRETRAIN_EPOCHS,
        "ESRGAN_ADV_WEIGHT": ESRGAN_ADV_WEIGHT,
        "ESRGAN_L1_WEIGHT": ESRGAN_L1_WEIGHT,
        "ESRGAN_SAM_WEIGHT": ESRGAN_SAM_WEIGHT,
        "ESRGAN_VI_WEIGHT": ESRGAN_VI_WEIGHT,
        "ESRGAN_ABLATION_PRETRAIN_EPOCHS": ESRGAN_ABLATION_PRETRAIN_EPOCHS,
        "ESRGAN_ABLATION_ADV_WEIGHT": ESRGAN_ABLATION_ADV_WEIGHT,
        "ESRGAN_ABLATION_SAM_WEIGHT": ESRGAN_ABLATION_SAM_WEIGHT,
        "ESRGAN_ABLATION_VI_WEIGHT": ESRGAN_ABLATION_VI_WEIGHT,
        "ESRGAN_ABLATION_LOSS_RAMP_EPOCHS": ESRGAN_ABLATION_LOSS_RAMP_EPOCHS,
        "ESRGAN_ABLATION_CONTENT_CLAMP": ESRGAN_ABLATION_CONTENT_CLAMP,
        "ESRGAN_L1_SAM_VI_PRETRAIN_EPOCHS": ESRGAN_L1_SAM_VI_PRETRAIN_EPOCHS,
        "ESRGAN_L1_SAM_VI_ADV_WEIGHT": ESRGAN_L1_SAM_VI_ADV_WEIGHT,
        "ESRGAN_L1_SAM_VI_WEIGHT": ESRGAN_L1_SAM_VI_WEIGHT,
        "ESRGAN_L1_SAM_VI_START_EPOCH": ESRGAN_L1_SAM_VI_START_EPOCH,
        "ESRGAN_L1_SAM_VI_RAMP_EPOCHS": ESRGAN_L1_SAM_VI_RAMP_EPOCHS,
        "ESRGAN_STABLE_VI_DENOM_EPS": ESRGAN_STABLE_VI_DENOM_EPS,
        "ESRGAN_STABLE_VI_BETA": ESRGAN_STABLE_VI_BETA,
        ...
        "EVAL_METRIC_PARALLEL": EVAL_METRIC_PARALLEL,
        "EVAL_METRIC_CPU_WORKERS": EVAL_METRIC_CPU_WORKERS,
        "TORCH_CPU_THREADS": TORCH_CPU_THREADS,
        "EVAL_TILE_BATCH_SIZE": EVAL_TILE_BATCH_SIZE,
    ...
    print(
        "ESRGAN ablation stabilization: "
        f"pretrain={ESRGAN_ABLATION_PRETRAIN_EPOCHS}, "
        f"adv={ESRGAN_ABLATION_ADV_WEIGHT}, "
        f"SAM={ESRGAN_ABLATION_SAM_WEIGHT}, "
        f"VI={ESRGAN_ABLATION_VI_WEIGHT}, "
        f"ramp={ESRGAN_ABLATION_LOSS_RAMP_EPOCHS}"
    )
    ...
    for scale in SCALES:
        ...
        if RUN_JOINT_5CH:
            for method in JOINT_METHODS_TO_RUN:
                ...
                # REVISION: skip method/scale/mode combinations not requested.
                if not revision_method_allowed(method, scale, mode):
                    print(f"[REVISION] Skip {method} x{scale} {mode} (per-method restriction)")
                    continue
                ...
                for loss_profile_id in get_loss_profile_ids_for_case(mode, method):
                    ...
                    if should_skip_eval(mode, method, scale, expected_rows=len(test_records)):
    ...
    # REVISION (D2): report excluded-scene (RMSE >= 1) counts/percentages.
    try:
        report_excluded_scenes(all_metric_rows, rmse_threshold=1.0)
    except Exception as e:
        print(f"[WARN] Excluded-scene report failed: {e}")

    # REVISION (B3): noise-robustness evaluation (no retraining). Runs only when
    # EVAL_NOISE_SIGMAS contains a value > 0 (enabled by the revision preset).
    try:
        run_revision_noise_experiment(test_records)
    except Exception as e:
        print(f"[WARN] Revision noise experiment failed: {e}")
