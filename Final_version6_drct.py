...
# by shell wrappers without editing the source. Defaults are unchanged.
...
PRESETS = {
    ...
    "revision_experiments": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["bicubic", "srcnn", "edsr", "rcan", "swinir", "esrgan", "hat", "dat", "drct", "edsr_wide"],
        "BANDWISE_METHODS_TO_RUN": ["bicubic", "edsr", "rcan", "swinir", "esrgan", "hat", "dat", "drct"],
        "RUN_JOINT_5CH": True,
        "RUN_BANDWISE_ABLATION": True,
        "EPOCHS": 50,
        "PATCHES_PER_EPOCH": 1000,
    ...
    # REVISION (B6): train + evaluate only the DRCT baseline, joint and
    # band-wise, at USER_SCALES = [2, 3, 4].
    "drct_only": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["drct"],
        "BANDWISE_METHODS_TO_RUN": ["drct"],
        "RUN_JOINT_5CH": True,
        "RUN_BANDWISE_ABLATION": True,
        "EPOCHS": 50,
        "PATCHES_PER_EPOCH": 1000,
    ...
    # REVISION (B6, Table 9): noise robustness for DRCT at x4. No retraining -
    # the x4 checkpoints from "drct_only" are reused and only the noisy-input
    # evaluation passes are run. Use together with SR_NOISE_METHODS=drct.
    "noise_drct": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["drct"],
        "BANDWISE_METHODS_TO_RUN": [],
    ...
    # REVISION (B6, Table 7): loss-term ablation rows for DRCT.
    "loss_ablation_drct": {
        "FAST_MODE": False,
        "JOINT_METHODS_TO_RUN": ["drct"],
        "BANDWISE_METHODS_TO_RUN": [],
...
TRAIN_MODE = {
    ...
    "drct": "scratch",       # B6: recent SOTA baseline (DRCT, CVPRW 2024)
...
elif EXPERIMENT_PRESET in ("noise_hat", "noise_drct"):
...
#   SR_NOISE_METHODS=drct
EVAL_NOISE_METHODS = (
    [m.strip() for m in os.environ["SR_NOISE_METHODS"].split(",") if m.strip()]
    if os.environ.get("SR_NOISE_METHODS")
    else (["hat"] if EXPERIMENT_PRESET == "noise_hat" else (["drct"] if EXPERIMENT_PRESET == "noise_drct" else ["edsr", "rcan", "esrgan"]))
)
EVAL_NOISE_SCALES = [4]
...
if EXPERIMENT_PRESET == "revision_experiments":
    ...
    LOSS_ABLATION_PROFILE_IDS = ["l1", "l1_sam", "l1_sam_vi"]
elif EXPERIMENT_PRESET == "loss_ablation_drct":
    LOSS_ABLATION_METHODS = ["drct"]
    LOSS_ABLATION_MODES = ["joint5ch"]
    LOSS_ABLATION_INCLUDE_FULL_LOSS_BASELINE = False
    LOSS_ABLATION_PROFILE_IDS = ["l1", "l1_sam", "l1_sam_vi"]
elif EXPERIMENT_PRESET in ("degradation_b2", "esrgan_retrain", "hat_bandwise_retrain", "noise_hat", "noise_drct", "drct_only"):
...
def get_infer_tile_settings(method):
    ...
    if str(method).lower() in ("hat", "dat", "drct"):
